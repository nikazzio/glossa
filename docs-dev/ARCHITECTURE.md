...
| `src/components/settings/DownloadSettingsTab.tsx` + `LibrariesSettingsTab.tsx` | Politica di scaricamento e biblioteche in Impostazioni (#422, #421). **Scaricamento**: il tetto di risoluzione fra 1000, 1500, 2000, 3000 e «la più grande disponibile», e il lato lungo delle miniature. **Biblioteche**: stessa forma delle impostazioni dei provider — una fila di pulsanti tondi con la sola sigla e il nome al passaggio del mouse, l'attivo in accento, un punto su chi ha valori propri — e sotto i valori di quella scelta (`LibraryProfileEditor.tsx`, tredici campi dichiarati in una tabella invece che in tredici blocchi uguali). Come in Scriptoria (`settings/panes/network.py`), i valori si **vedono ma non si toccano** finché l'interruttore «usa valori miei» resta spento: spegnerlo è il ripristino, perché sotto ci sono i valori compilati. Il comando per aggiungere un host fuori dal registro apre una casella nella stessa fila e parte dal profilo prudente letto dal backend. Il salvataggio **non è a ogni tasto**: un campo a metà non deve diventare la politica verso una biblioteca. Dopo il salvataggio la schermata rilegge l'elenco che il backend restituisce, così un valore riportato dentro i limiti si vede subito. Il tetto della **singola fonte** sta invece sulla scheda in Biblioteca (`workspace/SourceSizeCap.tsx`), che è dove D4 lo vuole. |
