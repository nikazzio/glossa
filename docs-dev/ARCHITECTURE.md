...
- **WSL2/WSLg**: tutti gli avvii Linux di Glossa sono su WSL2; `tauri:dev` imposta sempre `LIBGL_ALWAYS_SOFTWARE=1` + `WEBKIT_DISABLE_COMPOSITING_MODE=1`, oltre al fallback DMA-BUF WebKit già attivo su Linux. È il workaround verificato per finestra Tauri presente nella taskbar ma invisibile dopo riavvio; non avviare `tauri dev` direttamente.
