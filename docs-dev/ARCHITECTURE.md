...
| `stores/libraryStore.ts` | glossaries[], loadedForWorkspaceId, isLoaded, libraryScope | `libraryScope: 'workspace'\|'global'` (#213): `'workspace'` = dizionari del workspace da cui è aperta la Libreria (via `WorkspaceOverview`/`ProjectRailNext`/`ConfigDrawer`), editabili; `'global'` = Libreria generale aperta dal pulsante sempre visibile in `ShellNavFooter`, sfoglia tutti i workspace ma in sola lettura (niente crea/importa/rinomina/assegna/elimina), scheda Memoria frasi disattivata (nessun workspace di riferimento). `setShowLibraryPanel(show, tab?, scope?)` riparte sempre dal primo tab (Dizionari) se `tab` non specificato. `loadGlossaries(workspaceId)`/`reloadGlossaries(workspaceId)` accettano `string\|null`: con id filtra solo quel workspace (un glossario appartiene sempre a un solo workspace, #213), senza id resta lo sfoglio cross-workspace per lo scope globale. |
...
| `components/dashboard/AppDashboard.tsx` | Dashboard app-level (home dell'applicazione, 2026-07-18): riquadri panoramica (progetti totali, frammenti completati/totali, frasi in memoria, termini glossario — `getDashboardOverviewStats`, `countPhraseMemoryEntries`, `countGlossaryEntries`, tutti cross-workspace), "Richiede attenzione" (progetti con frammenti a giudizio scarso/critico o problemi aperti — `listProjectsNeedingAttention`, cross-workspace), Riprendi cross-workspace (ultimi 5 progetti, `listRecentProjectsAllWorkspaces`), attività recente pipeline (`listRecentPipelineRuns`, scope='pipeline' phase='end'), banner provider solo se mancante. Apertura progetto (Riprendi/Attenzione) via `projectStore.openProjectInWorkspace` (azione condivisa, vedi sotto). Non elenca più i workspace (già in barra laterale, righe workspace rimosse 2026-07-18). Sostituisce `WorkspaceHome` (rimossa). Pensata per crescere per-area: oggi solo Traduzioni ha numeri reali, Biblioteca/Trascrizioni si aggiungeranno come riquadri propri quando quelle aree saranno attive |
| `components/workspace/WorkspaceOverview.tsx` | Pagina del workspace attivo (vista `'workspace'`): identità/descrizione, azioni icon-only (Libreria, Configura, Elimina), lista progetti **solo di questo workspace** (`loadProjects` scoped su `activeWorkspace.id`), `+` nuovo progetto |
| `components/workspace/TranslationsArea.tsx` | Area Traduzioni (vista `'translations'`): TUTTI i progetti di TUTTI i workspace (`listAllProjects`, cross-workspace, non scoped), ognuno con badge nome workspace; click apre il progetto via `projectStore.openProjectInWorkspace` (attiva prima il workspace del progetto se diverso da quello corrente). Vista distinta da `WorkspaceOverview` (che mostra solo il workspace attivo) — corretto 2026-07-18 dopo bug segnalato (l'area sembrava "incollata" all'ultimo workspace visitato perché in realtà era scoped come `WorkspaceOverview`, non una vista realmente separata) |
...
glossaries / glossary_entries / project_glossaries
  ...
  Riuso tra workspace = fork esplicito (copia con nuovo nome), non condivisione live.
