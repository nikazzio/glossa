...
| `src-tauri/src/optimize/` | **Ottimizzazione locale delle immagini** (piano §5.7, decisione 3): lavoro della coda di classe **processore**, `JOB_TYPE = image_optimization`. Rilegge le pagine di **una** cartella di misura, le rimpicciolisce al lato lungo scelto con `images::resize_jpeg` e le ricomprime, sostituendo l'originale via `vault_io::stage_and_promote` — transito e spostamento atomico, così un'interruzione non lascia una pagina a metà. Riscrive **impronta** e **misura d'origine** nel file di lato (`Note::Downscaled`): senza la prima la verifica completa dichiarerebbe corrotto tutto il libro, senza la seconda si perderebbe che quella pagina è arrivata più grande. Rifà le miniature, che derivano dalle pagine. **Non tocca chi è già più piccolo** del lato lungo scelto, né chi non guadagnerebbe byte: ogni ricompressione perde qualcosa. Una cartella di misura per volta, mai «il libro», così le pagine prese a risoluzione piena di proposito (§5.6) non vengono schiacciate. Non è mai automatica. Comandi: `optimize_estimate` (quante pagine e quanto spazio, per la conferma), `enqueue_optimization`. Impostazioni: `optimize_long_edge` (2000, estremi 512-12000) e `optimize_jpeg_quality` (82, estremi 40-100), lette dal comando alla messa in coda. La conversione è controllata: troncare prima di guardare gli estremi faceva passare una qualità 300 come 44. Frontend: `services/optimizeService.ts`. |
