...
| `src-tauri/src/documents/` | Extract/export DOCX e PDF: extract_docx_text, extract_docx_markdown, export_markdown_docx, extract_pdf_text. Ogni comando valida il path (`validate_document_path`) contro le cartelle consentite (#367: Documenti/Download/Scrivania/cartelle app) **solo se** la preferenza `restrict_document_imports` è attiva — persistita in un file JSON sotto `app_config_dir` (`import_settings.json`), mai passata come argomento dalla webview: un comando `extract_*` non accetta più un booleano proprio, legge sempre lo stato salvato via `load_restrict_document_imports`, così un webview compromesso non può disattivare il controllo silenziosamente su una singola chiamata. Default disattivato. Restano comunque due floor non disattivabili, applicati in entrambe le modalità: (1) l'estensione deve essere fra quelle che il comando gestisce davvero — `docx` per gli extract DOCX, `pdf` per l'extract PDF (`has_allowed_extension`), così i comandi non sono usabili come primitiva di lettura file generica; (2) il path canonico non deve cadere in una posizione che contiene credenziali (`is_sensitive_path`, `SENSITIVE_RELATIVE_PATHS`: chiavi e portachiavi, cloud/infrastruttura, credenziali di rete e registri pacchetti, profili browser). `is_sensitive_path` fallisce chiuso: se la home non si risolve nega. Se `home_dir()` non è risolvibile, `validate_document_path` ricade sull'allowlist completa invece di lasciar passare la lettura. Comandi impostazione: `get_restrict_document_imports`, `set_restrict_document_imports` (letti/scritti da `services/storageConfigService.ts`, toggle in `StorageSettingsTab.tsx`). |
