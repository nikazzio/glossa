...
| `src/components/settings/DownloadSettingsTab.tsx` + `LibrariesSettingsTab.tsx` | Politica di scaricamento e biblioteche in Impostazioni (#422, #421). **Scaricamento**: il tetto di risoluzione fra 1000, 1500, 2000, 3000 e «la più grande disponibile», e il lato lungo delle miniature. **Biblioteche**: l'elenco del registro, ognuna apribile sui propri valori (`LibraryProfileEditor.tsx`, tredici campi dichiarati in una tabella invece che in tredici blocchi uguali), con il comando che la riporta ai valori compilati — disattivato per chi non è mai stato toccato — e la casella per aggiungere una voce su un host fuori dal registro, che parte dal profilo prudente letto dal backend. Il salvataggio **non è a ogni tasto**: un campo a metà non deve diventare la politica verso una biblioteca. Dopo il salvataggio la schermata rilegge l'elenco che il backend restituisce, così un valore riportato dentro i limiti si vede subito. Il tetto della **singola fonte** sta invece sulla scheda in Biblioteca (`workspace/SourceSizeCap.tsx`), che è dove D4 lo vuole. |
