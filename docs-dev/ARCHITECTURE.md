...
| `src-tauri/src/iiif/discovery.rs` | `discover_iiif(provider_key, input, page, fresh)` — `fresh` salta il risultato conservato e ripassa dalla biblioteca, e l'esito porta `cached_at` quando viene dalla cache, così l'interfaccia può dire di quando è: risolve manifest IIIF v2/v3 oppure cerca Internet Archive; restituisce un contratto normalizzato per la Dashboard e non persiste né scarica dati. |
...
| `src-tauri/src/vault/` | Deposito dei file scaricati dalle biblioteche (#217, blocco 1 PR 1). `layout.rs`: disposizione per **provenienza** — `providers/<chiave>/<id-versione>/` con `pages/<dimensione>/0001.jpg`, miniature e manifesto originale; percorsi sempre **relativi** alla radice, componenti validate contro la risalita. `verification.rs`: la verifica del deposito come lavoro (D5-bis; D5 modificata dal piano §5.4), classe **processore**. L'elenco da controllare si ricava **camminando le cartelle**, non dal database: rapida = il file c'è; completa = si confronta l'impronta con quella del file di lato, e un file **senza** riga si salta invece di dichiararlo corrotto. Miniature e manifesto non hanno impronta registrata: la prima si rigenera, il secondo è validato all'arrivo. Orfano è una **cartella di digitalizzazione che il database non conosce più**, contata in **file** e non in cartelle (è il numero che finisce nel messaggio di un'operazione irreversibile), e con una protezione: un elenco di opere conosciute **vuoto** — database appena creato, ripristino a metà — non rende orfano tutto il deposito. Non corregge niente: riferisce «integri 198 · mancanti 12 · corrotti 0 · orfani 3». L'esito resta leggibile **dopo**: `vaultService.lastVaultCheck()` legge l'ultimo controllo completato dal registro dei lavori e le impostazioni lo mostrano finché non se ne fa un altro — prima viveva nella riga del pannello e dopo un giorno spariva. Cancellarli è `delete_vault_orphans` (D5-bis), che **riguarda il deposito nel momento in cui si preme** invece di fidarsi del conto: fra il controllo e la cancellazione può essere finito uno scaricamento, e quei file non sono più orfani. **Nessun comando accetta un percorso**: la radice la legge il backend da `app_settings` (`configured_root` non arriva più dalla webview), e la finestra di scelta la apre Rust. `integrity.rs`: `scan_reader` legge la sorgente — file o byte già in memoria — **una volta sola** e ne ricava insieme impronta FNV-1a e validità strutturale (firma + terminatore, tenendo da parte primi e ultimi byte durante lo scorrimento) — coglie i troncamenti che un controllo di dimensione non vede, senza raddoppiare la lettura su gigabyte. `mod.rs`: radice configurabile a parte dal database, marcatore `.glossa-vault` per riconoscere un deposito da ricollegare o rifiutare una cartella con altro contenuto (rifiuto applicato anche in `initialize_vault`, non solo in `check_vault_folder`: sono comandi distinti e la schermata potrebbe saltare il primo), **radice assente distinta da file mancante**, `directory_stats` conta file e byte in una camminata sola. Le verifiche non abortiscono l'intero lotto per un `vault_path` malformato: quella riga diventa `invalid` e le altre proseguono. `absolute_path` accetta solo percorsi che il layout produce davvero (`layout::validate_vault_path`: radice `providers/`|`derived/`, componenti safe, estensione `jpg`|`json`|`pdf`) — i percorsi da verificare arrivano dal frontend e la radice è un'impostazione scrivibile dalla webview, quindi il divieto di risalita da solo lascerebbe i comandi usabili come oracolo di esistenza. La scelta della cartella passa dal dialogo nativo aperto dal backend (`choose_vault_folder`), e `check_vault_folder` — che accettava un percorso dal frontend e ci faceva una write probe — **non esiste più**: nessun comando del deposito accetta percorsi. Comandi: `get_vault_status`, `choose_vault_folder`, `use_default_vault_folder`, `initialize_vault`, `free_version_pages`, `enqueue_vault_verification`, `delete_vault_orphans` — la verifica per elenco di percorsi non esiste più: il disco è la verità, quindi l'elenco lo produce il deposito e confrontarlo con sé stesso non dice niente. Il confronto utile è file presenti contro conteggio atteso, e lo fa la scheda dell'opera leggendo l'inventario. Frontend: `services/vaultService.ts`. |
| `src-tauri/src/optimize/` | **Ottimizzazione locale delle immagini** (piano §5.7, decisione 3): lavoro della coda di classe **processore**, `JOB_TYPE = image_optimization`. Rilegge le pagine di **una** cartella di misura, le rimpicciolisce al lato lungo scelto con `images::resize_jpeg` e le ricomprime, sostituendo l'originale via `vault_io::stage_and_promote` — transito e spostamento atomico, così un'interruzione non lascia una pagina a metà. L'area di transito è **del lavoro** (`<versione>-optimize-<misura>`) e non della digitalizzazione: con una sola area questo lavoro e uno scaricamento sullo stesso libro si cancellavano i file a metà, e la coda li fa girare insieme perché uno occupa la rete e l'altro il processore. Riscrive **impronta** e **misura d'origine** nel file di lato (`Note::Downscaled`): senza la prima la verifica completa dichiarerebbe corrotto tutto il libro, senza la seconda si perderebbe che quella pagina è arrivata più grande. Rifà le miniature **alla misura scelta nelle impostazioni** — la stessa che legge lo scaricamento (`download::thumbnail_edge`) — perché derivano dalle pagine e rifarle a quella predefinita le rimpiccioliva alle spalle di chi l'aveva alzata. **Non tocca chi è già più piccolo** del lato lungo scelto, né chi non guadagnerebbe byte: ogni ricompressione perde qualcosa. Una cartella di misura per volta, mai «il libro», così le pagine prese a risoluzione piena di proposito (§5.6) non vengono schiacciate. Non è mai automatica. È **l'unica cosa che riduce**: dal 2026-08-19 il ripiego dello scaricamento conserva la dimensione piena com'è arrivata, quindi la misura d'origine nel file di lato è davvero quella con cui la pagina è arrivata dalla biblioteca. Comandi: `optimize_estimate` — quante pagine verrebbero **davvero** ridotte (non quante ce ne sono nella cartella: `forecast` legge le dimensioni dalle intestazioni, senza decodificare) e quanto si prevede di liberare col rapporto delle aree — e `enqueue_optimization`. Il pannello mostra pagine ridotte e byte liberati mentre gira. Impostazioni: `optimize_long_edge` (2000, estremi 512-12000) e `optimize_jpeg_quality` (82, estremi 40-100), lette dal comando alla messa in coda con conversione **controllata**: troncare prima di guardare gli estremi faceva passare una qualità 300 come 44. I due valori **per opera** stanno nella scheda di Biblioteca accanto al tetto per fonte e arrivano con la fase 2 del piano, non in una finestra al lancio. Frontend: `services/optimizeService.ts`. |
| `src-tauri/src/jobs/` | Orchestratore dei lavori in background (#218, blocco 1 PR 2). **Una coda sola dentro l'app**, avviata in `setup()`: `engine.rs` tiene i permessi per classe di risorsa (rete, processore, disco, servizi linguistici, documenti — limiti separati da `app_settings`, `0` = automatico), fa partire i lavori pronti e li fa decantare (`settle`); `store.rs` è tutta la SQL su `jobs`, con la **protezione della terminalità** nella clausola `WHERE status NOT IN ('completed','cancelled','error')` — un aggiornamento tardivo del gestore non resuscita un lavoro annullato; `mod.rs` porta stati, classi di risorsa, classificazione degli errori (`ErrorKind::is_retryable`, 403 ritentabile con raffreddamento lungo) e `BackoffProfile` (base 20 s, tetto 300 s, cooldown 600 s), che resta il ripiego per i tipi di lavoro senza profilo proprio: lo scaricamento allega all'errore l'attesa calcolata **dal profilo della biblioteca** (`JobContext::attempt` glielo permette), e i tentativi del lavoro sono `profile.max_attempts` (D16, D18). Pausa e annullamento sono **cooperativi**: `JobControl` alza un flag, il gestore lo guarda al confine dell'unità di lavoro e restituisce `Outcome::Paused`/`Cancelled`. `JobContext::report_phase` dichiara **cosa sta facendo adesso** dentro lo stato — chiave breve decisa dal tipo di lavoro (`starting`, `manifest`, `downloading`), tradotta dall'interfaccia, che mostra la chiave grezza per quelle che non conosce: così il pannello dice momento per momento a che punto è invece di un generico «in corso». `JobContext::report_progress` scrive al massimo una volta al secondo (D17) e `report_waiting` dichiara `waiting_reason = 'libraryLimits'` quando il lavoro è fermo per i limiti della biblioteca — fermo per cortesia e fermo per errore sono la stessa immobilità con significati opposti; `save_checkpoint` registra dove si è arrivati (D13). `NewJob.message` dà un nome al lavoro **già in coda**, prima che il gestore parta. Alla riapertura `recover_interrupted` porta i `cancelling` a `cancelled`, i riprendibili a `paused` (o in coda se `auto_resume_downloads` è acceso e la classe è rete) e gli altri in coda col progresso azzerato — **niente riparte da solo**. Ogni cambiamento esce come evento `jobs:updated`. **I log** hanno una forma sola — `job <evento> id=… key=value`, così una ricerca su `id=` ricostruisce la storia di un lavoro: `error` per ciò che è finito male e richiede una decisione, `warn` per ciò che si rimedia da solo (tentativo rimandato, misura rifiutata, raffreddamento, punto non salvato), `info` per il ciclo di vita (in coda, avviato, in pausa, ripreso, annullato, finito, recupero all'avvio, limiti letti), `debug` per il dettaglio a carta e le attese di cortesia. Il taglio fra `debug` e `info` è quello fra build di sviluppo e applicazione compilata, deciso in `lib.rs`; `RUST_LOG` scavalca entrambi. Le scritture passano dal `DbWriteCoordinator` come tutto il resto. Gestori registrati: solo i due finti delle build di sviluppo (`testing.rs`), il primo reale è lo scaricamento della PR 4. Tipi di lavoro registrati: `source_download` (uno solo per digitalizzazione: le miniature si ricavano dalle carte, non si scaricano), `vault_verification`, più i due finti delle build di sviluppo. Comandi: `create_job`, `list_active_jobs` (i non finiti **più i terminati nelle ultime 24 ore**, altrimenti la sezione «terminati oggi» si svuota a ogni riavvio), `get_job`, `pause_job`, `resume_job`, `cancel_job`, `retry_job`, `clear_finished_jobs`. Frontend: `services/jobsService.ts`. |
...
| `src-tauri/src/download/` | Scaricamento delle fonti (#218; riscritto contro `SCARICAMENTO_E_DEPOSITO.md` §5.1-§5.4). `courtesy.rs` tiene pausa, limite a raffica, concorrenza **per host** e raffreddamento dopo 403/429 — i contatori stanno sull'host, il profilo lo dichiara il provider (D18). `fetch.rs` classifica: 403 `Throttled`, 429 `RateLimited` con `Retry-After`, 404/410 `NotFound`, **400/501 `SizeRejected`** — rifiuto *della misura*, valutato prima dei 5xx perché 501 sta in quell'intervallo e non è un guasto — 5xx `Transport`, con tre tentativi ravvicinati sulla singola richiesta — e se un 5xx insiste sulla stessa pagina fino all'**ultimo** tentativo del lavoro si prova la dimensione piena per quella pagina sola, poi la si salta **senza scrivere niente**: salire con l'errore lascerebbe il libro troncato, perché nessuna ripresa arriverebbe alle pagine successive, ma la riga «non servita» è per i rifiuti dichiarati (404/410) — scriverla per un guasto renderebbe il libro irraggiungibile per una settimana, e una rete caduta la scriverebbe su ogni pagina. Un lavoro che non porta niente per un guasto è di trasporto e si ritenta; se invece la biblioteca ha dichiarato di non servire niente, no. `manifest.rs` legge Presentation 3.0 e 2.1. **`sizing.rs`** sostituisce la negoziazione per gruppo: il descrittore si legge **una volta per libro** e risponde a una domanda sola — la biblioteca tiene pronti i dimezzamenti (`SizingRule::{Halvings,ExactWidth,Full}`)? Poi la misura si calcola dalle dimensioni del manifesto — 47 gruppi misurati sul campo predetti 47 volte su 47. Costo: una richiesta per libro, 4,3 s su un lavoro di ore. Al primo rifiuto della misura si passa a `Full` per il resto del libro e **si conserva la pagina come arriva**: nessuna ricompressione alle spalle dell'utente, che è la regola che il §5.7 dichiarava già per l'ottimizzazione e che il ripiego contraddiceva (deciso il 2026-08-19). Costa spazio, e lo spazio si recupera a freddo con l'ottimizzazione locale; il dettaglio buttato non torna. Le dimensioni nel file di lato dicono quali pagine sono più grandi del nome della cartella. Quando la regola è già `Full` il ripiego non ripete la stessa richiesta, che con il tetto a `max` sarebbe una richiesta buttata per ogni pagina non servita. **`sidecar.rs`** è il sostituto delle righe per pagina: `pages/<misura>/pages.jsonl`, una riga in coda dopo lo spostamento atomico, ultima riga per indice vincente, impronta FNV-1a, e le due note che vanno raccontate (`Downscaled`, `NotServed`). Un file **senza** riga è una pagina presente di impronta ignota: si conta, la verifica completa la salta. **`inventory.rs`** legge le cartelle come inventario e le espone: `version_inventory`, `library_inventory`. La misura **principale** è quella con più pagine, così una cartella `max` con tre file su 328 non fa sembrare incompleto un libro completo (§5.6). `handler.rs` è il ciclo in sette passi: nessun punto di ripresa salvato (riprendere = rileggere la cartella), nessuna riga `assets` per pagine e miniature, cartella di misura vuota a fine lavoro ⇒ errore. Restano transito → validazione → `rename` (D16-bis) e la miniatura ricavata dai byte già in memoria (D6). **`progress.rs`** tiene i conti e la stima: il tempo per pagina viene dalle **ultime dieci** pagine arrivate e non dalla media da inizio lavoro, che le pagine ritrovate sul disco rendevano ottimista e un raffreddamento pessimista per tutto il resto del libro. `handler_it.rs` fa girare il lavoro vero contro una biblioteca finta (`wiremock`). Comando: `enqueue_source_download`. |
| `src-tauri/src/httpcache/` | **Una cache sola per tutto quello che viene dalla rete** (D8, piano `SCARICAMENTO_E_DEPOSITO.md` §5.5): copertine, miniature remote, risposte delle ricerche e — quando ci sarà il visore — le pagine dei libri non scaricati. **Chiave: la richiesta** (`request.rs`, `CacheRequest::{Remote,Page,Search}` ridotta a testo canonico prima dell'impronta FNV-1a; `BTreeMap` per i filtri, perché l'ordine deve essere stabile). **Valore: i byte.** Come nel deposito **il disco è la verità**: un file per richiesta sotto `<cartella dati>/cache/<due lettere>/<impronta>` più un `.meta` con tipo di contenuto e scadenza, l'ultimo uso è la data del file toccata alla lettura, l'occupazione si somma camminando la cartella, lo scarto ordina per data e butta finché non si scende sotto il tetto — **nessun indice da riparare dopo una chiusura brusca**, e cancellare la cartella a mano è legittimo. Sta nella cartella dati e **mai nel deposito**: il deposito conserva ciò che è stato scaricato di proposito, questa si svuota senza conseguenze ed è esclusa dai backup. Solo le ricerche scadono (predefinito 24 ore, `search_cache_ttl_hours`); le immagini le governa il solo tetto (predefinito 512 MB, `cache_max_bytes`). Catena di risoluzione in `commands.rs`: deposito alla misura → **cache** → deposito a misura maggiore, rimpicciolito sul momento e messo in cache → rete **passando dalla cortesia**. La cache sta prima della riduzione e non dopo: è la riduzione a riempirla, e cercarla dopo significherebbe rifarla a ogni voltata senza rileggere mai quella di prima. Una voce **senza il suo file di lato non si serve** — senza scadenza, una ricerca che valeva un giorno varrebbe per sempre — quindi si scarta. Byte e file di lato si scrivono in transito e poi al loro posto. Il client HTTP è **riusato** per coppia di scadenze: uno nuovo per copertina è una connessione nuova per copertina. `cached_image` restituisce byte grezzi (`tauri::ipc::Response`): un `Vec<u8>` serializzato costa tre o quattro volte i byte che trasporta. Se la cartella dati non si risolve la cache **non esiste** e i comandi degradano invece di cadere. Identificativo e misura di una pagina passano da `layout::safe_component` prima di diventare cartelle (#405). **Mostrare e possedere restano separati**: quello che sta qui non è mai contato come scaricato. Comandi: `cached_image`, `cache_usage`, `apply_cache_cap` (il tetto abbassato vale subito, non alla prossima copertina), `clear_cache`. Frontend: `services/cacheService.ts`, `hooks/useCachedImage.ts`, `components/common/CachedThumbnail.tsx`. |
