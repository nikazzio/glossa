...
| `components/workspace/LibraryCatalogArea.tsx` | Il catalogo con **due ambiti** (#213, `uiStore.catalogScope`): «solo le opere di questo workspace» — che è l'apertura predefinita, perché non si legge da un altro workspace senza chiederlo — e «tutte», che resta il modo di ritrovare un'opera e collegarla. Il comando collega/scollega sta su ogni scheda ed è lo stesso nei due versi, con l'accento quando è collegata; `listLibraryCatalog(workspaceId, onlyLinked)` porta il collegamento come colonna, così non serve una seconda lettura per scheda. |
