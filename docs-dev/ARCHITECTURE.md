...
| `src-tauri/src/vault/` | Deposito dei file scaricati dalle biblioteche (#217, blocco 1 PR 1). `layout.rs`: disposizione per **provenienza** — `providers/<chiave>/<id-versione>/` con `pages/<dimensione>/0001.jpg`, miniature e manifesto originale; percorsi sempre **relativi** alla radice, componenti validate contro la risalita. `integrity.rs`: impronta FNV-1a a blocchi e validazione per decodifica (firma e terminatore), che coglie i file troncati che un controllo di dimensione non vede. `mod.rs`: radice configurabile a parte dal database, marcatore `.glossa-vault` per riconoscere un deposito da ricollegare o rifiutare una cartella con altro contenuto, **radice assente distinta da file mancante**. Comandi: `get_vault_status`, `check_vault_folder`, `initialize_vault`, `expected_version_paths`, `verify_files_present`, `verify_files_integrity`, `free_version_pages`. Frontend: `services/vaultService.ts`. |
