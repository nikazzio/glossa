...
- **Rail home = una sola selezione di navigazione**: gerarchia a tre livelli — `Dashboard` voce standalone in cima (home app-level, fuori dalle aree), sotto lo **switcher workspace** (riga col nome attivo + menu cambia/crea: è contesto, mai voce attiva di nav), sotto le **aree** del workspace attivo. Dashboard + aree = semantica radio: sempre esattamente una attiva, default Dashboard, click sull'attiva = no-op, mai deselezione. La Dashboard è app-level (Riprendi cross-workspace, attività recente, riepilogo workspace, stato provider, strip 2.0); l'inventario completo con gestione vive nell'area dedicata. Si torna alla Dashboard dalla sua voce in barra, non da back dedicati. Cambiare workspace non cambia la vista corrente.
