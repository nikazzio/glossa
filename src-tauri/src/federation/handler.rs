...
use crate::iiif::{
    discovery::{Gate, SearchPage},
    search::SearchEndpoints,
};
use crate::jobs::{
    engine::{JobContext, JobHandler},
    ErrorKind, JobError, Outcome, Recovery, ResourceClass,
};
...
impl JobHandler for SearchJob {
    ...
    async fn run(&self, ctx: JobContext) -> Result<Outcome, JobError> {
        let config: SearchConfig =
            serde_json::from_str(&ctx.config).map_err(|e| failure(e.to_string()))?;
        let started = std::time::Instant::now();
        let log = |event: &str, details: serde_json::Value| {
            super::log_event(
                event,
                &config.search_id,
                &ctx.id,
        ...
        if ctx
            .with_database(|conn| store::saved_page(conn, &config))
            .await
            .map_err(failure)?
        {
        ...
        let criteria = ctx
            .with_database(|conn| store::criteria(conn, &config.search_id))
            .await
            .map_err(failure)?;
        let provider = crate::iiif::find_provider(&config.provider_key)
            .ok_or_else(|| failure("federation.providerUnavailable".into()))?;
        let handler = provider
            .search_handler
            .filter(|_| provider.supports_search && provider.is_enabled)
            .ok_or_else(|| failure("federation.providerUnavailable".into()))?;
        let profile = ctx
            .with_database(|conn| {
        ...
        let cache_entry = if config.fresh {
            ...
            crate::httpcache::commands::lookup_with_age(&self.0, &request).and_then(
                |(bytes, at)| {
                    serde_json::from_slice::<SearchPage>(&bytes)
                        .ok()
                        .map(|page| (page, at))
                },
            )
        ...
        log(
            "search.cache.checked",
            serde_json::json!({"hit":cached.is_some(),"fresh":config.fresh}),
        );
        let work = async {
        ...
        let page = loop {
            if ctx.cancel_requested() {
                log("search.execution.cancelled", serde_json::json!({}));
                return Ok(Outcome::Cancelled);
            }
            if ctx.pause_requested() {
                log("search.execution.paused", serde_json::json!({}));
        ...
        ctx.with_database(|conn| store::save_page(conn, &ctx.id, &config, &page))
            .await
            .map_err(failure)?;
        log(
            "search.page.committed",
            serde_json::json!({"received":page.results.len(),"hasMore":page.has_more,"durationMs":started.elapsed().as_millis()}),
        );
