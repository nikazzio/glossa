...
-- ── Ricerca federata: ricerche, esecuzioni e pagine di risultati ──
...
CREATE INDEX IF NOT EXISTS search_executions_run
  ON search_executions(search_id, provider_key, generation DESC);

-- `received`/`has_more` si scrivono quando la pagina arriva: elencare le
-- ricerche non deve riaprire e rileggere ogni payload salvato.
-- `execution_id` tiene l'esecuzione che ha davvero scritto la pagina, così un
-- tentativo fallito non si attribuisce i risultati prodotti dal suo retry.
CREATE TABLE IF NOT EXISTS search_pages (
  ...
  execution_id TEXT REFERENCES search_executions(id),
