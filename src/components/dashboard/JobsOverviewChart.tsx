...
export function JobsOverviewChart({ jobs }: { jobs: Job[] }) {
  ...
  const series = [
    ...
    { label: t('jobs.queued'), count: jobs.filter((j) => j.status === 'queued').length, color: 'bg-editorial-muted' },
    { label: t('jobs.paused'), count: jobs.filter((j) => j.status === 'paused').length, color: 'bg-editorial-muted/50' },
  ...
  return <dl className="space-y-2" aria-label={t('overview.openJobs')}>
    {series.map((item) => <div key={item.label} className="grid grid-cols-[minmax(0,1fr)_2.5rem_2rem] items-center gap-2 text-xs">
      <dt className="truncate text-editorial-muted">{item.label}</dt>
      <Tooltip label={`${item.label}: ${item.count}`} className="w-full">
        <span className="block h-[3px] w-10 overflow-hidden rounded-full bg-editorial-textbox" aria-hidden="true"><span className={`block h-full rounded ${item.color}`} style={{ width: `${item.count / maximum * 100}%` }} /></span>
      </Tooltip>
