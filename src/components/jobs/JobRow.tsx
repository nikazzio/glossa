...
/**
 * Una riga di lavoro: descrizione, avanzamento, comandi, e i dettagli completi
 * sotto quando si apre.
 *
 * La stessa riga serve il pannello in basso e l'elenco completo della
 * Panoramica: sono due elenchi dello stesso oggetto, e un lavoro che si legge
 * in due modi diversi a seconda di dove lo si guarda sarebbe un lavoro diverso.
 * Cambia solo cosa fa il comando in coda: nel pannello toglie la riga dalla
 * vista, nell'elenco completo la elimina dal deposito.
 */
export function JobRow({ job, onRemove, removeLabel, singleColumn = false }: {
  job: Job;
  onRemove?: () => void;
  removeLabel?: string;
  /** In una colonna stretta i dettagli stanno su una colonna sola: due
   *  colonne lì dentro lasciano a ogni valore quattro centimetri e si legge
...
{detail.speed !== undefined && detail.speed > 0 && ` · ${humanSize(detail.speed)}/s`}
      ...
      >
        <div className="overflow-hidden">{open && <JobDetails job={job} detail={detail} singleColumn={singleColumn} />}</div>
      </div>
    </div>
  );
}
...
function JobDetails({ job, detail, singleColumn }: { job: Job; detail: JobDetail; singleColumn: boolean }) {
  const { t } = useTranslation();
  ...
  return (
    <div className="flex flex-col gap-2 border-t border-editorial-border pt-2">
      <FieldGroup title={t('jobs.detail.groupWork')} fields={work} singleColumn={singleColumn} />
      {lastUnit.length > 0 && (
        <FieldGroup title={t('jobs.detail.groupLast')} fields={lastUnit} singleColumn={singleColumn} />
      )}
    </div>
  );
}

function FieldGroup({ title, fields, singleColumn }: {
  title: string;
  fields: DetailField[];
  singleColumn: boolean;
}) {
