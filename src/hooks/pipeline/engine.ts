...
import { runJudgeForChunk } from './runJudge';
...
export type ProviderCheck = { provider: string; model: string; label: string };
 ...
 */
export async function ensureProvidersReady(checks: ProviderCheck[], t: Translate): Promise<boolean> {
...
function getChunkMemoryBlock(chunkId: string): string | undefined {
...
function warnAsyncFailure(scope: string, error: unknown, context?: Record<string, unknown>) {
  const message = error instanceof Error ? error.message : String(error);
  logger.warn(scope, { ...context, error: message });
}

function persistPipelineStatus(
...
async function computeBlobAssignments(
  ...
  try {
    ...
    useChunksStore.getState().setBlobAssignments(assignments);
  } catch (error: unknown) {
 ...
 */
export async function executePipelineForChunk(
...
export async function runChunkExecution(
  chunkId: string,
  t: Translate,
  ...
  if (!(await ensureProvidersReady(buildProviderChecks(config), t))) return;
  useChunksStore.getState().clearCancelRequest();
  useChunksStore.getState().setIsProcessing(true);

  await computeBlobAssignments(config, useChunksStore.getState().chunks, t);

  const freshChunk = useChunksStore.getState().chunks.find((c) => c.id === chunkId) ?? chunk;
  const outcome = await executePipelineForChunk(freshChunk, t, {
    memoryBlock: options?.memoryBlock ?? getChunkMemoryBlock(chunkId),
  });

  useChunksStore.getState().setIsProcessing(false);
  useChunksStore.getState().clearCancelRequest();
...
export async function runPipeline(t: Translate): Promise<void> {
  if (useChunksStore.getState().isProcessing) return;
  ...
  await computeBlobAssignments(config, liveChunks, t);
  ...
  try {
    for (const chunk of liveChunks) {
      const outcome = await executePipelineForChunk(chunk, t, { batchMode, memoryBlock: getChunkMemoryBlock(chunk.id) });
...
export function cancelPipeline(t: Translate): void {
...
export async function rerunChunkWithMemory(
  chunkId: string,
  selectedMatches: PhraseMemoryMatch[],
  t: Translate,
): Promise<void> {
  const memoryBlock = buildMemoryInjection(selectedMatches);
  await runChunkExecution(chunkId, t, { memoryBlock: memoryBlock ?? undefined });
}
